...
export class AccountingEngineService {
  ...
  private saveGstEntries(transactionId: number, lines: EngineTransactionLine[], dateStr: string) {
    ...
    // Sum up input vs output gst, track rate
    let totalInput = 0;
    let totalOutput = 0;
    let inputRate = 5.0;
    let outputRate = 5.0;
    ...
    for (const line of lines) {
      if (line.gstType === 'input' && line.gstAmount) {
        totalInput += line.gstAmount;
        if (line.gstRate) inputRate = line.gstRate;
      }
      if (line.gstType === 'output' && line.gstAmount) {
        totalOutput += line.gstAmount;
        if (line.gstRate) outputRate = line.gstRate;
      }
    }

    if (totalInput > 0) insertGst.run(transactionId, 'input', totalInput, inputRate, month, year);
    if (totalOutput > 0) insertGst.run(transactionId, 'output', totalOutput, outputRate, month, year);
  }
  ...
  private generateInvoice(transactionId: number, transactionNo: string, event: EngineEvent): number {
    ...
    // Build a unique invoice number. If a collision exists (e.g. from legacy data),
    // append an incrementing counter until we find a free slot.
    const parts = transactionNo.split('-');
    const baseSuffix = parts.slice(1).join('-') || `${Date.now()}`;
    let invoiceNo = `INV-${baseSuffix}`;
    let attempt = 0;

    const checkExisting = this.db.prepare('SELECT 1 FROM invoices WHERE invoice_no = ?');
    while (checkExisting.get(invoiceNo)) {
      attempt++;
      const randomPart = crypto.randomInt(0, 10000).toString().padStart(4, '0');
      invoiceNo = `INV-${baseSuffix}-${attempt}-${randomPart}`;
    }
  ...
  private generateTransactionNo(type: string): string {
    ...
    // Use crypto-secure random for the sequence part to prevent collisions
    const crypto = require('crypto');
    const randomPart = crypto.randomInt(0, 10000).toString().padStart(4, '0');
    return `${prefix}-${datePart}-${timePart}-${msPart}-${randomPart}`;
  }

  private calculateOutstanding(contactId: number): number {
    // Outstanding = normalized balance for contact.
    // Logic: Match by tl.account_id OR by tl.contact_id/t.contact_id restricted to AR/AP accounts.
    // This ensures we catch historical entries that might be missing specific links, while ignoring Cash/Bank lines.
    ...
    `).get(contact.account_id, contactId, contactId) as any;
