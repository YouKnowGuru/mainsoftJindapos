...
export function ContactsPage({ type }: ContactsPageProps) {
  ...
  const loadContacts = async () => {
    try {
      const result = await window.electronAPI.contacts.getAll(type);
      if (result) {
        setContacts(result);
      }
    } catch (error) {
  ...
  const viewLedger = async (contact: Contact) => {
    try {
      const result = await window.electronAPI.contacts.getLedger(contact.id);
      if (result) {
          ...
          <div className="bg-white rounded-2xl w-full max-w-2xl p-6 sm:p-8 my-auto shadow-2xl animate-in fade-in zoom-in-95 duration-200">
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                ...
                <div className="space-y-1.5">
                  <label className="block text-xs font-bold text-slate-600 uppercase tracking-wide">TPN / GST</label>
                  <input
                    type="text"
                    value={formData.gstNumber}
                    onChange={(e) => setFormData({ ...formData, gstNumber: e.target.value })}
                    className="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-bhutan-maroon/20 focus:border-bhutan-maroon focus:bg-white transition-all text-sm font-medium text-slate-800 uppercase"
              ...
              <div className="space-y-1.5">
                ...
                <textarea
                  ...
                  className="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-bhutan-maroon/20 focus:border-bhutan-maroon focus:bg-white transition-all text-sm font-medium text-slate-800 min-h-[80px] resize-none"
            ...
            <div className="p-6 border-b border-slate-100 bg-slate-50/50 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              <div>
                <div className="flex items-center gap-3 mb-2">
                  ...
                  <h3 className="text-xl font-bold text-slate-800">Statement of Account</h3>
                </div>
                <div className="flex items-center gap-3">
