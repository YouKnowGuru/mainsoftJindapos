...
export function GSTPage() {
  ...
  const loadGSTSummary = async () => {
    try {
      const result = await window.electronAPI.gst.getSummary(selectedMonth, selectedYear);
  ...
  const loadGSTReturns = async () => {
    try {
      const result = await window.electronAPI.gst.getReturns();
      if (result.success && result.data) {
    ...
    try {
      ...
      const content = `
        ...
        <table style="width: 100%; border-collapse: collapse; margin-bottom: 30px;">
          ...
          <tbody>
            <tr>
              <td rowspan="2" style="font-weight: bold; background: #f8fafc;">Purchases</td>
            ...
            <tr>
              <td rowspan="2" style="font-weight: bold; background: #f8fafc;">Sales</td>
              <td>Taxable Sales</td>
      ...
      const result = await window.electronAPI.print.report(title, content);
      if (result.success) {
      ...
      {gstSummary && (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          <div className="bg-white rounded-2xl shadow-sm border border-slate-50 p-6 group hover:shadow-md transition-shadow">
            ...
            <p className="text-2xl font-black text-bhutan-maroon">{formatCurrency(gstSummary.gstInput)}</p>
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-slate-50 p-6 group hover:shadow-md transition-shadow">
            ...
            <p className="text-2xl font-black text-bhutan-orange">{formatCurrency(gstSummary.gstOutput)}</p>
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-slate-50 p-6 group hover:shadow-md transition-shadow">
            ...
            <p className={`text-2xl font-black ${gstSummary.gstPayable > 0 ? 'text-red-500' : 'text-slate-600'}`}>
              {formatCurrency(gstSummary.gstPayable)}
            </p>
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-slate-50 p-6 group hover:shadow-md transition-shadow">
            ...
            <p className="text-2xl font-black text-bhutan-gold/80">{formatCurrency(gstSummary.gstCredit)}</p>
          ...
          <div className="bg-white rounded-xl shadow-sm p-6">
            ...
            <div className="space-y-4">
              <div className="flex justify-between py-2 border-b border-gray-100">
                ...
                <span className="font-medium">{formatCurrency(gstSummary.taxablePurchases)}</span>
              </div>
              <div className="flex justify-between py-2 border-b border-gray-100">
                <span className="text-gray-600">Exempt Purchases</span>
                <span className="font-medium">{formatCurrency(gstSummary.exemptPurchases)}</span>
              </div>
              <div className="flex justify-between py-2 border-b border-gray-100">
                <span className="text-gray-600">GST Input</span>
                <span className="font-medium text-emerald-600">{formatCurrency(gstSummary.gstInput)}</span>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-xl shadow-sm p-6">
            ...
            <div className="space-y-4">
              <div className="flex justify-between py-2 border-b border-gray-100">
                ...
                <span className="font-medium">{formatCurrency(gstSummary.taxableSales)}</span>
              </div>
              <div className="flex justify-between py-2 border-b border-gray-100">
                <span className="text-gray-600">Exempt Sales</span>
                <span className="font-medium">{formatCurrency(gstSummary.exemptSales)}</span>
              </div>
              <div className="flex justify-between py-2 border-b border-gray-100">
                <span className="text-gray-600">GST Output</span>
                <span className="font-medium text-blue-600">{formatCurrency(gstSummary.gstOutput)}</span>
              </div>
            </div>
          </div>
        </div>
            ...
            {gstReturns.map((returnData, index) => (
              <tr key={index} className="border-t border-gray-100 hover:bg-gray-50">
                ...
                <td className="py-3 px-4 text-center">
                  <span className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium ${returnData.is_filed
